...
from unet import UNet, SwinTransformerSys, MobileNetV3, SegFormer
from utils.data_loading import BasicDataset, CarvanaDataset
from utils.dice_score import dice_loss
from utils.segLoss import IoULoss
...
def train_model(
    ...
    val_loader = DataLoader(val_dataset, shuffle=False, batch_size=1, num_workers=1, pin_memory=True)
    ...
    for epoch in range(1, epochs + 1):
            ...
            for batch in train_loader:
                ...
                with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
                    ...
                    if model.n_classes == 1:
                        ...
                        loss, iou = criterion(masks_pred, true_masks, returnIOU=True)
                        iou_train += iou
                        loss += dice_loss(
                ...
                experiment.log({
                    ...
                    'train iou': iou_train / len(train_loader),
                    'step': global_step,
                    'epoch': epoch
                })
                ...
                if division_step > 0:
                    if global_step % division_step == 0:
                        ...
                        dice, iou = evaluate(model, val_loader, device, amp)
                        scheduler.step(dice)

                        logging.info(
                            'Dice: {:.2f} \t IoU: {:.3f}'.format(dice, iou))
                        try:
                            experiment.log({
        ...
        if save_checkpoint:
            ...
            torch.save(state_dict, str(dir_checkpoint /
                       'checkpoint_epoch{}_mask.pth'.format(epoch)))
...
def get_args():
    ...
    parser.add_argument('--load', '-f', type=str,
                        # default="checkpoints/checkpoint_epoch200.pth", help='Load model from a .pth file')
                        default="", help='Load model from a .pth file')
    ...
    parser.add_argument('--dataset', '-d', type=str,
                        # default='pothole600resizepng_new_tr_val', help='Dataset Name') 
                        default='QasimSWIN', help='Dataset Name')
                        # default='pothole600_real', help='Dataset Name')
    # parser.add_argument('--img-size', '-i', type=int, default=400, help='Image size')
